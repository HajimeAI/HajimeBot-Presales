...
declare_id!("4XCwWuXcPyev8bJEMSKs4Ubk5FsoXBjobKyRipG5gaJL");
...
mod hajime_ticket {
    ...
    pub fn set_payment(ctx: Context<SetPayment>, token_addr: Pubkey, price: Option<u64>) -> Result<()> {
        ...
        let old_payment = token_state.allow_tokens.iter_mut().find(|t| t.token_addr == payment.mint);

        if old_payment.is_none() {
            if token_state.allow_tokens.len() >= MAX_ALLOW_PAYMENTS {
                token_state.allow_tokens.remove(0);
            }

            token_state.allow_tokens.push(Payment{
                token_addr: payment.mint.clone(),
                price,
            });

            emit!(event::PaymentAddEvent {
                token_addr: payment.mint,
                allow_num: token_state.allow_tokens.len() as u8
            });
        } else {
            let old_payment = old_payment.unwrap();
            old_payment.price = price;
        }
    ...
    pub fn buy_nft(ctx: Context<BuyNFT>, token_addr: Pubkey, price: u64) -> Result<()> {
        ...
        let support_payment = token_state.allow_tokens.iter().find(|&t| t.token_addr == source.mint);
        if support_payment.is_none() {
            return Err(ProgramError::InvalidArgument.into());
        }
        
        let support_payment = support_payment.unwrap();
        if support_payment.price.is_some() && support_payment.price.unwrap() != price {
            return Err(ProgramError::InvalidArgument.into());
        } else if support_payment.price.is_none() && price != token_state.price {
            return Err(ProgramError::InvalidArgument.into());
        }
